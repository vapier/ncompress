...
#		define FAST
#	elif USERMEM >= (433484 + SACREDMEM)
#		define BITS 16
#	elif USERMEM >= (229600 + SACREDMEM)
#		define BITS 15
#	elif USERMEM >= (127536 + SACREDMEM)
#		define BITS 14
#	elif USERMEM >= (73464 + SACREDMEM)
#		define BITS 13
#	else
#		define BITS 12
#	endif
		...
		do
		{
			...
			if (!stcode && bytes_in >= checkpoint && fcode.e.ent < FIRST)
			{
				...
				{ /* shift will overflow */
					...
					rat = (bytes_in << 8) /
						(bytes_out + (outbits >> 3)); /* 8 fractional bits */
				if (rat >= ratio)
					ratio = (int)rat;
				else
				{
					...
					reset_n_bits_for_compressor(
						n_bits, stcode, free_ent, extcode, maxbits);
				}
			}
		...
		while (inbits > posbits)
		{
			...
			if (code == CLEAR && block_mode)
			{
				...
				reset_n_bits_for_decompressor(
					n_bits, bitmask, maxbits, maxcode, maxmaxcode);
				goto resetbuf;
			}
